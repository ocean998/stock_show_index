...
class MyFigure(FigureCanvas):
    def __init__(self,width=13, height=4, dpi=200):
    ...
    def plot_macd(self, code = 'sz000725', cycle = '60'):
        ...
        self.axes.plot(idx_macd.loc[:, 'time'], idx_macd.loc[:, 'dif'], idx_macd.loc[:, 'time'], idx_macd.loc[:, 'dea'])
        self.axes.plot(idx_macd.loc[:, 'time'], idx_macd.loc[:, 'dif'], color='white')
        self.axes.plot(idx_macd.loc[:, 'time'], idx_macd.loc[:, 'dea'], color='yellow')
...
class MyPlot():
    ...
    def show( self ):
        # plt.plot(self.macd)
        idx_macd = pd.read_csv('C:/Users/Administrator/PycharmProjects/stock_show_index/macd.csv')
